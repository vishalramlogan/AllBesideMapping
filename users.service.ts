...
export class UsersService {
  ...
  passwordResetdb(username: string, password: string, passwordConfirmation: string, securityQuestionAnswer: string){
    const passon = {username: username, password: password, passwordConfirmation: passwordConfirmation,
       ...
       this.http.put<{token: string}>("http://localhost:3000/api/users", passon).subscribe(
         (response) => {
          localStorage.setItem('token', response.token);
          localStorage.setItem('theUser', username);
          this.router.navigate(['', username]);
  ...
  getUserInfo(){
    ...
    this.http.get<{mappings: []}>(("http://localhost:3000/api/usersinfo?username="+username),{ headers })
    .subscribe(messageData => {
      this.Response.emit(messageData.mappings);
    }); 
}

getUserInfoAdmin(username: string){
  const headers = {'Authorization': `Bearer ${this.token}`};
  this.http.get<{mappings: []}>(("http://localhost:3000/api/usersinfo?username="+username),{ headers })
  .subscribe(messageData => {
    this.Response.emit(messageData.mappings);
...
getListUsers(){
  const username = localStorage.getItem('theUser');
  const headers = {'Authorization': `Bearer ${this.token}`};
  console.log("http://localhost:3000/api/"+username)
  this.http.get<{username: []}>(("http://localhost:3000/api/users/"+username),{ headers })
  .subscribe(messageData => {
    this.Response.emit(messageData.username);
  }); 
}

deletedb(username: string){
  const headers = {'Authorization': `Bearer ${this.token}`};
  const options = {
    body: { username: username }
  };
  this.http.delete<{}>("http://localhost:3000/api/users/Administrator", { headers, ...options }).subscribe(
    () => {
      location.reload()
    },
    (error) => {
